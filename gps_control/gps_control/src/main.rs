...
use gps_interface::gps_control::{GPSMode, GPS_DATA_DIR};
...
async fn main() -> std::io::Result<()> {
    ...
    /*let gps_control = gps_interface::gps_control::GPSControl::new(
        Some(&cli.gpsd_server),
        Some(cli.gpsd_port),
        Some(cli.gps_usb_port),
        Some(cli.gps_tty_port), /*Some(cli.output_port)*/
    )
    .start();

    let mut gps_interface = gps_interface::gps_interface::GPSInterface::new(
        Some(&cli.gpsd_server),
        Some(cli.gpsd_port),
        socket_monitor.clone(),
    );
    ...
    /* match cli.mode {
        Modes::RTKRover {
            ...
            gps_control.do_send(GPSMode::RtcmIn(
                username,
                password,
                server,
                mount_point,
        ...
        Modes::RTKBase {
            ...
            port,
            survey_dwell_time,
            survey_position_accuracy,
            fixed_ecef_x,
            fixed_ecef_y,
            fixed_ecef_z,
            fixed_ecef_accuracy,
        } => {
            gps_control.do_send(GPSMode::Base(
                username,
                password,
                server,
                mount_point,
        ...
        Modes::PPPMode {
            ...
            gps_control.do_send(GPSMode::RAW(
                data_directory,
                filename,
                interval,
                number_of_collections,
